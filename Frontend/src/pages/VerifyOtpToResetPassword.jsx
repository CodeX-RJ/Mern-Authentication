  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await axios.post(
        "https://rj-mern-authentication.onrender.com/api/auth/verify-reset-otp",
        { email: userEmail, passwordResetOtp: finalOtp }
      );

      alert(res.data.message);
      if (res.data.success) navigate("/change-password");
  ...
  return (
    <div className="w-screen min-h-screen grid place-items-center p-4 bg-gray-50 mt-24">
      <div className="w-full max-w-lg rounded-lg shadow-lg shadow-blue-600 border bg-gradient-to-r from-indigo-500 to-purple-600 border-black p-6 flex flex-col items-center">
        ...
        <h1 className="text-3xl font-extrabold text-white mb-2">Reset Password Verification</h1>
        <p className="text-center text-lg font-semibold text-black mb-4 px-2">
        ...
        <div className="flex justify-center gap-2 sm:gap-3 mb-4">
          {otp.map((digit, index) => (
            <input
              ...
              className="h-12 w-10 sm:h-14 sm:w-14 text-xl sm:text-2xl font-bold
                text-center border-2 border-black bg-gray-200 rounded-md
